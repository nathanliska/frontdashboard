...
- **#66 — The only external availability alert cannot fire (2026-08-07).** `SiteDown` (critical) and `CertificateExpiringSoon` both name `probe_success{job="blackbox-public"}`, and no `blackbox_exporter` is installed — so both evaluate to nothing and stay silent forever. Every other target sits inside the Docker network, so the whole board reads green while Caddy, the frontend container or Cloudflare is what broke: exactly the failure these two exist for is the one they cannot see. Found by *executing* each rule against Prometheus rather than parsing it — a rule naming a series nothing produces is valid PromQL, so a syntax check passes it and a rename disables an alert rather than breaking it. `PublicProbeMissing` is the built-in canary and fires 10m after `alerts.yml` is imported, so the gap announces itself rather than hiding. Fixing it is a Community Applications install plus the job in [observability/README.md](../observability/README.md). Note this is the *opposite* case to the exporters deferred below: an unreachable app cannot report that it is unreachable, so nothing self-instrumented can substitute. *(Small — one install on the box)*
...
- **Exporters for the infrastructure we don't own** (`redis_exporter`, `postgres_exporter`) — when a
  ...
  cannot move. Measured 2026-08-07 and holding: `used_memory` 1.35 MiB against the 64 MiB cap, 2%.
  A `redis_exporter` was built and reverted the same day rather than re-argued — `noeviction` is what
  makes it redundant, since a full store refuses writes and so reaches the app as a failed one, which
  `rate_limit_store_degraded` and `sse_publish_failures` already report. Neither is blocked on
  anything; they are one compose service and one scrape target each, on a box whose Prometheus config
