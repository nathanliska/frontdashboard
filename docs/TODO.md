...
- **#28◐ — User-discovery privacy.** Done: exclude deleted/unverified, ≥2-char floor, escape LIKE wildcards. Remaining: decide whether full emails belong in results, rate-limit the endpoint, and pick prefix vs substring semantics with a matching index. **Sharpened by open registration** — `/api/users/search` is reachable by any authenticated user, and anyone with a working mailbox can become one, so today it is an email-harvesting endpoint over the whole user table for any stranger who signs up. *(Small)*
...
- **#18 — Commit to one visibility model, schema-enforce it.** A polymorphic share table and ~35 `resource_type` branches across `services/shares.py` still model direct list/event shares that no route can reach (those `/shares` endpoints are deliberate 409 stubs). Replace with `dashboard_shares(dashboard_id, user_id, role)` + cascading FKs and delete the child-share code — **a deletion, not a rewrite**. Depends on [ADR-001](adr/ADR-001-per-resource-sharing.md). *(Large)*
