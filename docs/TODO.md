...
- **#41◐ — Split routes / remove dead surfaces.** Done: route-level lazy loading (main chunk 526→310 kB), knip CI gate, dead components/`App.css` removed. Remaining: remove the dead `CalendarReminder` schema and the vestigial `membership.*` event variants (or complete those features explicitly). *(Small-Medium)*
