...
from app.sse.choreography import commit_and_broadcast
...
# Same password oracle and Argon2 cost as change_password, so the same bound.
@limiter.limit("5/minute")
async def delete_own_account(
    ...
    """Delete the caller's account: purge what only they could reach, tombstone the rest.

    Refused while they own a dashboard others can see — hand it over or delete it first, so
    nobody loses access as a side effect of someone else leaving.
    """
    ...
    if shared:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Hand over or delete the dashboards you share first: {', '.join(shared)}",
        )
    revoked_ids, fanouts = await delete_account(db, current_user)
    await commit_and_broadcast(db, actor_id=current_user.id, fanouts=fanouts)
    drop_session_streams(revoked_ids)
    _clear_auth_cookies(response)
