...
async def test_each_left_frame_is_addressed_to_its_own_dashboard_and_keeps_the_name_it_carried(
    ...
    # The members hear the name they knew; the rows it came from do not keep it.
...
async def test_the_lock_makes_a_second_submit_wait_and_then_find_nothing_to_do(
    ...
    """Two real transactions: the loser must block on the row rather than re-run the purge."""
    ...
    # Unlocked, this read would have returned True here — the winner has not committed yet.
...
async def test_the_dashboard_lock_holds_a_redemption_off_until_the_purge_has_landed(
    concurrent_sessions: tuple[AsyncSession, AsyncSession, uuid.UUID],
) -> None:
    """The statement `accept_invite` opens with must wait, and then find the dashboard gone.

    Without the lock it reads the row while the purge is uncommitted and grafts a share onto a
    dashboard about to vanish; with it, the redeemer takes the ordinary "no longer valid" path.
    """
    first, second, user_id = concurrent_sessions
    ...
    try:
        ...
        waiting = asyncio.create_task(second.execute(select(Dashboard).where(Dashboard.id == dashboard_id).with_for_update()))
        await asyncio.sleep(0.2)
        assert not waiting.done()

        await delete_account(first, owner)
        await first.commit()
        assert (await waiting).scalar_one_or_none() is None
    finally:
...
async def test_a_dashboard_cannot_be_handed_to_an_account_on_its_way_out(
    auth_client: AsyncClient, db_session: AsyncSession, accounts: MemberFactory
) -> None:
    """The transfer re-reads the target under lock, so a deletion racing it cannot strand a board."""
    dashboard = await create_dashboard(auth_client, name="Household")
    member = await accounts("member@example.com", display_name="Member")
    await share_dashboard(auth_client, dashboard["id"], member, "editor")
    member_id = (await current_user(member))["id"]

    row = (await db_session.execute(select(User).where(User.id == member_id))).scalar_one()
    row.deleted_at = datetime.now(UTC)
    await db_session.flush()

    resp = await auth_client.post(f"/api/dashboards/{dashboard['id']}/owner", json={"user_id": member_id})
    assert resp.status_code == 409, resp.text
    assert (await auth_client.get(f"/api/dashboards/{dashboard['id']}")).json()["user_id"] == (await current_user(auth_client))["id"]
