...
async def test_restore_is_refused_to_a_viewer(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """Restore is a write, so it takes edit — a viewer must not undo someone else's delete."""
    dashboard = await create_dashboard(auth_client)
    event = await create_calendar_event(auth_client, dashboard["id"])

    viewer = await accounts("restore-viewer@example.com")
...
async def test_restore_is_refused_to_a_stranger(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)
    event = await create_calendar_event(auth_client, dashboard["id"])
    assert (await auth_client.delete(f"/api/calendar/events/{event['id']}")).status_code == 204

    stranger = await accounts("restore-stranger@example.com")
