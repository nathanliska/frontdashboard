...
async def test_a_member_can_leave_and_loses_access(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)

    editor = await accounts("leave-editor@example.com")
...
async def test_a_viewer_can_leave_too(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """Leaving is not an edit on the dashboard, so the viewer role must not gate it."""
    dashboard = await create_dashboard(auth_client)

    viewer = await accounts("leave-viewer@example.com")
...
async def test_leaving_twice_is_a_404(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)

    member = await accounts("leave-twice@example.com")
...
async def test_a_stranger_gets_a_404(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)

    stranger = await accounts("leave-stranger@example.com")
    assert await _leave(stranger, dashboard["id"]) == 404


async def test_the_owner_sees_the_share_gone(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)

    member = await accounts("leave-gone@example.com")
    await share_dashboard(auth_client, dashboard["id"], member, "editor")
...
async def test_leaving_is_logged_as_left_not_removed(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """The feed distinguishes "you left" from "you were removed" by share_action, like "joined"."""
    dashboard = await create_dashboard(auth_client)

    member = await accounts("leave-logged@example.com")
    await share_dashboard(auth_client, dashboard["id"], member, "editor")
    assert await _leave(member, dashboard["id"]) == 204
...
async def test_leaving_clears_a_home_pointing_at_the_dashboard(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """The preference sweep must cover self-removal, or the home screen 404s on next load."""
    dashboard = await create_dashboard(auth_client)

    member = await accounts("leave-home@example.com")
    await share_dashboard(auth_client, dashboard["id"], member, "editor")
...
async def test_leave_then_rejoin_then_leave_again(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """The cycle must be repeatable: a second leave finds the re-issued share, not stale state."""
    dashboard = await create_dashboard(auth_client)

    member = await accounts("leave-cycle@example.com")
    await share_dashboard(auth_client, dashboard["id"], member, "editor")
    assert await _leave(member, dashboard["id"]) == 204
    await share_dashboard(auth_client, dashboard["id"], member, "editor")
    assert await _leave(member, dashboard["id"]) == 204
