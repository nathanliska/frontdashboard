...
from tests.helpers import MemberFactory, create_dashboard, create_list, create_list_item, share_dashboard


async def test_private_list_is_only_visible_to_owner(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    ...
    other = await accounts("other@example.com")
    resp = await other.get(f"/api/lists/{lst['id']}")
    assert resp.status_code == 404


async def test_shared_dashboard_editor_can_add_items(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """A user shared as editor on the dashboard can add items to lists."""
    dashboard = await create_dashboard(auth_client)
    lst = await create_list(auth_client, dashboard["id"])

    other = await accounts("editor@example.com")
...
async def test_shared_dashboard_viewer_cannot_mutate(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """A viewer on the dashboard can read lists but not edit them."""
    dashboard = await create_dashboard(auth_client)
    lst = await create_list(auth_client, dashboard["id"])

    other = await accounts("viewer@example.com")
...
async def test_no_share_returns_404_for_other_user(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)
    lst = await create_list(auth_client, dashboard["id"])
    other = await accounts("no-share@example.com")
    resp = await other.patch(f"/api/lists/{lst['id']}", json={"name": "Blocked"})
    assert resp.status_code == 404


async def test_dashboard_share_crud_endpoints(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    """Dashboard share CRUD works for dashboards that own lists."""
    dashboard = await create_dashboard(auth_client)

    other = await accounts("share-crud@example.com")
    await share_dashboard(auth_client, dashboard["id"], other, "viewer")
...
async def test_viewer_empty_item_patch_is_rejected_not_written(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)
    lst = await create_list(auth_client, dashboard["id"])
    item = await create_list_item(auth_client, lst["id"])

    other = await accounts("viewer-empty@example.com")
    await share_dashboard(auth_client, dashboard["id"], other, "viewer")
...
async def test_list_details_requires_dashboard_access(auth_client: AsyncClient, accounts: MemberFactory) -> None:
    dashboard = await create_dashboard(auth_client)
    await create_list(auth_client, dashboard["id"])
    stranger = await accounts("details-stranger@example.com")
